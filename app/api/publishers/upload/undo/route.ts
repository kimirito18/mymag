...
`);
    ...
    return createRouteErrorResponse(error, "failed to load upload undo state", {
      databaseMessage: "データベースに接続できないため出版社CSV Undo情報を読み込めません。",
    });
  }
}
...
`);
    ...
    const payload = parseJsonObject<UploadUndoPayload>(targetLog.before_data, { publishers: [] });
...
`);
    ...
    return createRouteErrorResponse(error, "failed to apply upload undo", {
      databaseMessage: "データベースに接続できないため出版社CSV Undoを実行できません。",
    });
  }
}
