alter table public.magazine_issues
  add column if not exists total_issue_number text not null default '';

update public.magazine_issues
set
  total_issue_number = coalesce(nullif(issue_number_displayed, ''), nullif(volume_number, ''), ''),
  volume_number = '',
  issue_number_displayed = ''
where total_issue_number = ''
  and issue_number = ''
  and sub_issue_number = ''
  and volume_number <> ''
  and issue_number_displayed = volume_number;
