...
alter table public.magazine_issues
  add column if not exists publication_frequency text not null default '月刊';

update public.magazine_issues
set
  publication_frequency = '月刊',
  media_format = 'print',
  updated_at = now()
where record_status <> 'deleted'
  and (
    publication_frequency is distinct from '月刊'
    or media_format is distinct from 'print'
  );

alter table public.magazine_issues
  drop constraint if exists magazine_issues_publication_frequency_required;

alter table public.magazine_issues
  add constraint magazine_issues_publication_frequency_required
  check (btrim(publication_frequency) <> '');
